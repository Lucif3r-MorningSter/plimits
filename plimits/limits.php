...
class trackersLimit
{
	...
	{
		global $restrictedTrackersURL;
		global $profileMask;
		self::trace('Try to retrieve trackers list');
		$fname = getSettingsPath()."/trackers.lst";
		...
		if($client->fetch($restrictedTrackersURL) && ($client->status==200))
		{
			$this->trackers = explode("\n",$client->results);
			self::trace('Got '.count($this->trackers).' items.');
			$fp = fopen( $fname.'.tmp', "a" );
			if($fp!==false)
			{
				if(rCache::flock( $fp ))
				{
					ftruncate( $fp, 0 );
					fputs( $fp, $client->results );
		        		fflush( $fp );
					flock( $fp, LOCK_UN );
        				fclose( $fp );
